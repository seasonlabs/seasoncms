 ...
 */
function active_preprocess_page(&$vars) {
  include_once(drupal_get_path('theme', 'active') .'/styles.inc');
  $vars['styles'] .= active_style_css(active_get_style_info());
  $vars['styles_ie6'] = active_style_ie6();

  $settings = theme_get_settings('active');
 ...
 */
function active_preprocess_node(&$vars) {
 ...
 */
function active_style_css($info) {
 ...
 */
function active_status_messages($display = NULL) {
  $output = '';
  $first = TRUE;
